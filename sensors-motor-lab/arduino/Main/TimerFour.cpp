...
void TimerFour::setPeriod(long microseconds)
{
    ...
    if (cycles < RESOLUTION)
        clockSelectBits = _BV(CS40); // no prescale, full xtal
    else if ((cycles >>= 3) < RESOLUTION)
        clockSelectBits = _BV(CS41); // prescale by /8
    else if ((cycles >>= 3) < RESOLUTION)
        clockSelectBits = _BV(CS41) | _BV(CS40); // prescale by /64
    else if ((cycles >>= 2) < RESOLUTION)
        clockSelectBits = _BV(CS42); // prescale by /256
    else if ((cycles >>= 2) < RESOLUTION)
        clockSelectBits = _BV(CS42) | _BV(CS40); // prescale by /1024
    else
        cycles = RESOLUTION - 1, clockSelectBits = _BV(CS42) | _BV(CS40); // request was out of bounds, set as maximum
...
{
    if (pin == 8)
        TCCR4A &= ~_BV(COM4A1); // clear the bit that enables pwm on PE3
    if (pin == 6)
        TCCR4A &= ~_BV(COM4B1); // clear the bit that enables pwm on PE4
    if (pin == 7)
        TCCR4A &= ~_BV(COM4C1); // clear the bit that enables pwm on PE5
}
